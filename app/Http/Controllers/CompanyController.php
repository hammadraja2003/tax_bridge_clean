...
use App\Http\Resources\BusinessConfigurationResource;
class CompanyController extends Controller
{
    ...
    public function storeOrUpdate(Request $request)
    {
        ...
        try {
            ...
            $request->validate([
                ...
                'bus_name' => 'required|string|max:255',
                'bus_address' => 'required|string|max:255',
                'bus_province' => 'required|string|max:255',
                'bus_account_title' => 'nullable|string|max:255',
                'bus_account_number' => 'nullable|string|max:255',
                'bus_reg_num' => 'nullable|string|max:255',
                'bus_contact_num' => 'nullable|string|max:20',
    ...
    {
        $busConfigId = $request->bus_config_id;
        /** If not found */
        if (!$busConfigId) {
            return  errorResponse("bus_config_id is required", 400);
        }
        $config = BusinessConfiguration::with('scenarios')
        ...
        $rules = [
            ...
            'bus_account_number' => 'required|string|max:255',
            'bus_reg_num' => 'required|string|max:255',
            'bus_contact_num' => 'required|string|max:20',
            'bus_contact_person' => 'required|string|max:255',
            'bus_IBAN' => 'required|string|max:255',
            'bus_acc_branch_name' => 'required|string|max:255',
            'bus_acc_branch_code' => 'required|string|max:255',
            'bus_logo' => $config && $config->bus_logo
                ? 'nullable|mimes:jpg,jpeg,png,svg|max:2048'
                : 'required|mimes:jpg,jpeg,png,svg|max:2048',
            'fbr_env' => 'required|in:sandbox,production',
        ...
        try {
            ...
            logActivity(
                $action,
                ucfirst($msg),
                $action === 'add' ? $config->toArray() : ['old' => $oldData, 'new' => $config->toArray()],
                $config->id,
                'business_configurations'
            );
            DB::commit();
            return successResponse($config, 200, $msg);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    //             message: 'Invalid business configuration ID',
    //             status: 404
    //         );
    //     }
    ...
    public function fetchConfiguration(Request $request)
    {
        ...
        return successResponse(
            data: [
                'config'            => new BusinessConfigurationResource($config),
                'scenarios'         => $scenarios,
                'selectedScenarios' => $selectedScenarios,
            ],
            message: 'Configuration fetched successfully'
