...
<body>
    ...
    <div class="container" id="invoiceArea">
            ...
            <div class="logo">
                @php
                    ...
                    if ($config && $config->bus_logo) {
                        try {
                            $imageData = Storage::disk($disk)->get($config->bus_logo);
                            $extension = pathinfo($config->bus_logo, PATHINFO_EXTENSION) ?: 'png';
                            $base64Logo = 'data:image/' . $extension . ';base64,' . base64_encode($imageData);
                        } catch (\Throwable $e) {
                            $base64Logo = null;
                        }
                    }
                    if (!$base64Logo) {
                        $localPath = public_path('assets/images/logo/tax-bridge.svg');
                        if (File::exists($localPath)) {
                            $base64Logo = 'data:image/svg+xml;base64,' . base64_encode(File::get($localPath));
                        }
                    }
                @endphp
                @if ($base64Logo)
                    <img src="{{ $base64Logo }}" alt="Logo" style="height:45px;">
                @else
                    <img src="{{ asset('assets/images/logo/tax-bridge.svg') }}" alt="Logo" style="height:45px;">
                @endif
            </div>
        </div>
        ...
        <table style="width: 100%; margin-top: 20px; border-collapse: collapse;">
            <tr>
                ...
                <td style="width: 50%; vertical-align: top; text-align: left; padding-right: 20px;">
                    ...
                    <h4 style="margin: 10px 0 5px; text-decoration: underline;">Bank Details</h4>
                    <p style="margin: 0;">
        ...
        <div class="footer" style="text-align: center;">
           Office:{{ $invoice->seller->bus_address }}
        </div>
    </div>
</body>
